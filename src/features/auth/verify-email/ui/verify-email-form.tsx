            ...
            <InputOTPGroup className="w-full justify-between gap-2">
              {Array.from({ length: OTP_LENGTH }).map((_, index) => (
                <InputOTPSlot
                  key={index}
                  index={index}
                  className="data-[active=true]:border-primary/60 data-[active=true]:ring-primary/25 size-11 flex-1 rounded-xl border border-white/10 bg-white/5 text-base font-semibold first:rounded-xl first:border-l last:rounded-xl data-[active=true]:ring-3"
                />
              ))}
            </InputOTPGroup>
          </InputOTP>
